...
FASTLinName = [ Name{:}, '.lin'];   % FAST (.lin) filename
...
line = fgetl( FileID );
RotSpeed   = 1./str2num( line(55:68) );    % in (rad/s)
line = fgetl( FileID );
line = fgetl( FileID );
line = fgetl( FileID );
line = fgetl( FileID );
...
%   then ignore the two column headers:
    line = fgetl( FileID );
    Azimuth(L) = str2num( line(20:27) );   % in (deg)
...
clear  FASTLinName FileID line Row Col BldCol GeAzCol DrTrCol EqlCol I J K L Tmp OK OutName Real1 Real2 Imag1 Imag2 ans;
%bjj end of change 10/2/08
