...
%
   ...
   for iCh=1:nFatigueChannels
         ...
         for iFile=1:nFiles           
            ...
            FileBasedResults(iFile,iCh).binnedLifetimeCycles = 0;  % this will end up being multiple arrays of varying lengths
            FileBasedResults(iFile,iCh).binnedDamage      =cell(1,nSNSlopes);
         end
   ...
   for iFile = 1:nFiles  
      ...
      if ( iFile <= DLCs(1).NumFiles + DLCs(2).NumFiles )
         ...
         if ( Fatigue.UserDistrib )
            timeFactor = extrapolate_time_factor2( iDLC, windChannelMeans(iFile), Fatigue.WSin, Fatigue.WSout, Fatigue.DistribVars, ...
                                               Fatigue.DesignLife, ...
                                               Fatigue.TimePP, Fatigue.TimeIdle,  Fatigue.Availability, Fatigue.DistribVarValues(:,iFile), Fatigue.DistribTable );
         else
            timeFactor = extrapolate_time_factor( iDLC, windChannelMeans(iFile), Fatigue.WSin, Fatigue.WSout, ...
                                               ...
                                               Fatigue.TimePP, Fatigue.TimeIdle, Fatigue.WSProb, Fatigue.Availability );
         end
      else
          ...
          % cycles = generate_cycles( peaks, Fatigue.UCMult, double(Fatigue.ChanInfo(iCh).LUlt), double(lmf) );
            ...
            if ( Fatigue.BinCycles )
               ...
               for c=1:length( cycleRanges )
                  index                    = min(ceil( cycleRanges(c)/loadRangeBinWidth ), nLoadRangeBins);  % Prevents numerical issues                   
               ...
               FileBasedResults(iFile,iCh).binnedLifetimeCycles = lifetimeCycles;  
            ...
            for iSlope = 1:Fatigue.ChanInfo(iCh).NSlopes
               ...
               if ( Fatigue.BinCycles )    
                  ...
                  FileBasedResults(iFile,iCh).binnedDamage{iSlope} = lifetimeCycles./cyclesToFailure;
