...
%% Search file for @Restore command
%Fatigue.RestorePeaks = false;
%for PLine=1:size( ParamFile{1}, 1 )
%   if ( ~isempty( regexpi( ParamFile{1}{PLine}, '@Restore' ) ) )
%      %Found a restore
%      Fatigue.RestorePeaks = true;

%      % temp = textscan( ParamFile{1}{PLine}, '%s %s', 1 );
%      temp = read_value( ParamFile{1}{PLine}, 'string' , 2, '', '' ) ;
%      Fatigue.RestoreFilename = cell2mat(temp(2));
%      break
%   end % if
%end % for PLine
   % Search file for @Save command
%Fatigue.SavePeaks = false;
%for PLine=1:size( ParamFile{1}, 1 )
%   if ( ~isempty( regexpi( ParamFile{1}{PLine}, '@Save' ) ) )
%      %Found a save
%      Fatigue.SavePeaks = true;

%      % temp = textscan( ParamFile{1}{PLine}, '%s %s', 1 );
%      temp = read_value( ParamFile{1}{PLine}, 'string' , 2, '', '' ) ;
%      Fatigue.SaveFilename = cell2mat(temp(2));
%      break
%   end % if
%end % for PLine
...
%=================================================================
% Read the information for distributions. 
%=================================================================
Fatigue = [];
...
Fatigue.UserDistrib         = cell2mat( read_value( ParamFile{1}{PLine+01}, 'logical', 1, 'UserDistrib'  , 'User defined distribution?', fidEcho                                                                       ) );
Fatigue.weibullShapeFactor  = cell2mat( read_value( ParamFile{1}{PLine+02}, 'float'  , 1, 'weibullShapeFactor'  , 'Weibull shape factor. If WeibullShape=2, enter the mean wind speed for WeibullScale.', fidEcho      ) );
temp                        = cell2mat( read_value( ParamFile{1}{PLine+03}, 'float'  , 1, 'weibullScaleFactor'  , 'Weibull scale factor. If WeibullShape<>2.  Otherwise, enter the mean wind speed.', fidEcho          ) );
...
Fatigue.WSin                = cell2mat( read_value( ParamFile{1}{PLine+04}, 'float'  , 1, 'WSin'       , 'Cut-in wind speed for the turbine.', fidEcho                                                                 ) );
Fatigue.WSout               = cell2mat( read_value( ParamFile{1}{PLine+05}, 'float'  , 1, 'WSout'      , 'Cut-out wind speed for the turbine.', fidEcho                                                                ) );
Fatigue.WSmax               = cell2mat( read_value( ParamFile{1}{PLine+06}, 'float'  , 1, 'WSmax'      , 'Maximum  value for the wind-speed bins.', fidEcho                                                            ) );
Fatigue.WSMaxBinSize        = cell2mat( read_value( ParamFile{1}{PLine+07}, 'float'  , 1, 'WSMaxBinSize'   , 'The max width of a wind-speed bin.', fidEcho                                                             ) );
Fatigue.nDistribVars        = cell2mat( read_value( ParamFile{1}{PLine+08}, 'integer', 1, 'nDistribVars', 'The number of independent variables in the user-specified distribution.', fidEcho                           ) );
Fatigue.DistribFile         = cell2mat( read_value( ParamFile{1}{PLine+09}, 'string' , 1, 'DistribFile' , 'Filename of the user-supplied distribution table.', fidEcho                                                 ) );

Fatigue.DistribTable = 0;

if ( Fatigue.UserDistrib )
   
   if (nargin > 1)
         
      fileseps = strfind(Fatigue.DistribFile, filesep);

      if ( isempty(fileseps) || fileseps(1) == length(Fatigue.DistribFile) )
         Fatigue.DistribFile = sprintf('%s', dataDirectory, Fatigue.DistribFile);
      end 

   end
   
   [Fatigue.DistribVars, Fatigue.DistribTable] = read_joint_pdf(Fatigue.DistribFile);
   
   if ( Fatigue.nDistribVars > 7 ) 
      beep
...
Fatigue.WrShortTermTxt      = cell2mat( read_value( ParamFile{1}{PLine+10}, 'logical', 1, 'WrShortTermTxt'  , 'Write DELs to plain-text files?', fidEcho                                                               ) );
Fatigue.WrShortTermXLS      = cell2mat( read_value( ParamFile{1}{PLine+11}, 'logical', 1, 'WrShortTermXLS'  , 'Write DELs to an Excel workbook?', fidEcho                                                              ) );
Fatigue.WrLifeTxt           = cell2mat( read_value( ParamFile{1}{PLine+12}, 'logical', 1, 'WrLifeTXT'  , 'Write lifetime results to plain-text files?', fidEcho                                                        ) );
Fatigue.WrLifeXLS           = cell2mat( read_value( ParamFile{1}{PLine+13}, 'logical', 1, 'WrLifeXLS'  , 'Write lifetime results to an Excel workbook?', fidEcho                                                       ) );
Fatigue.EquivalentFrequency = cell2mat( read_value( ParamFile{1}{PLine+14}, 'float'  , 1, 'EquivalentFrequency'  , 'The frequency of the damage equivalent load (Hz).', fidEcho                                        ) );
Fatigue.DEL_AsRange         = cell2mat( read_value( ParamFile{1}{PLine+15}, 'logical', 1, 'DEL_AsRange', 'true = report DELs as a range value,  false = report as a one-sided amplitude', fidEcho                      ) );
Fatigue.DEL_Type            = cell2mat( read_value( ParamFile{1}{PLine+16}, 'integer', 1, 'DEL_Type'   , '1 = fixed mean, 2 = zero mean, 3 = compute both', fidEcho                                                    ) );
Fatigue.GoodmanFlag         = cell2mat( read_value( ParamFile{1}{PLine+17}, 'integer', 1, 'GoodmanFlag', '0 = no Goodman, 1 = use Goodman, 2 = compute with and without', fidEcho                                      ) );
...
%   end
   ...
   if ( FileInfo.DLCs(1).NumFiles > 0 )
      if ( Fatigue.UserDistrib )
         distribVarValues1   = zeros(Fatigue.nDistribVars,FileInfo.DLCs(1).NumFiles);
      else
         distribVarValues1 = 0;
      end
      ...
      for iFile=1:FileInfo.DLCs(1).NumFiles
         ...
         if ( Fatigue.UserDistrib )
            ...
            Fmt = [ '%q', repmat( ' %f', 1, Fatigue.nDistribVars ) ];
      
            temp = textscan( ParamFile{1}{PLine+iFile}, Fmt, 1 );
            fileName = cell2mat(temp{1});
            for iVar=1:Fatigue.nDistribVars
               distribVarValues1(iVar,iCount)  = temp{1+iVar};
            end
         else
            fileName = cell2mat( read_value( ParamFile{1}{PLine+iFile}, 'string' , 1, '', '', fidEcho ) );
         end
         
         FileList1{iCount,1} = fileName;
         iCount              = iCount + 1;
   ...
   if ( FileInfo.DLCs(2).NumFiles > 0 )
      if ( Fatigue.UserDistrib )
         distribVarValues2   = zeros(Fatigue.nDistribVars,FileInfo.DLCs(2).NumFiles);
      else
         distribVarValues2 = 0;
      end
      ...
      for iFile=1:FileInfo.DLCs(2).NumFiles
         if ( Fatigue.UserDistrib )
            ...
            fileName = cell2mat(temp{1});
            for iVar=1:Fatigue.nDistribVars
               distribVarValues2(iVar,iCount)  = temp{1+iVar};
            end
         else
            fileName = cell2mat( read_value( ParamFile{1}{PLine+iFile}, 'string' , 1, '', '', fidEcho ) );
         end
         FileList2{iCount,1} = fileName;
         iCount              = iCount + 1;
      end % for File
      totalFiles = totalFiles + FileInfo.DLCs(2).NumFiles;     
      FileList = [FileList1; FileList2];
      Fatigue.DistribVarValues = [distribVarValues1 ; distribVarValues2];
   else
      FileList = FileList1;
      Fatigue.DistribVarValues = distribVarValues1; 
   end
